    ...
    var brando = {
                        ...
                        e.preventDefault();
                            ...
                            // console.log( $(this).next().offset().top );
                                ...
                                if( _wheelDelta < 0 ){ //마우스 휠 아래로 돌리면 메뉴는 부드럽게 사라지고
                                    ...
                                    if( idx > 0 ){
                                        if( idx == n-1 ){ //마지막 이면
                                            $('html,body').stop().animate({scrollTop: $(this).prev().find('.wheel-event').last().offset().top },600);
                                        }
                                        else{
        ...
        section01Fn: function(){
            ...
            var setId2 = 0;
                ...
                //한개는 이전(현재실행했던) 슬라이드 번호
                //또 한개는 지금 클릭한 번호
                function mainNextSlideFn(){
                    slide.css({zIndex:1});
                    if(imsi!==null){ //페이지 버튼 클릭시 실행 구문
                ...
                function mainPrevSlideFn(){
                    slide.css({zIndex:1}).find('li').stop().animate({width:25+'%'},0); //초기화
                    //이전 슬라이드
                ...
                $('.pageBtn').each(function(idx){
                    $(this).on({
                        click:  function(event){
                            ...
                            //현재 슬라이드번호 보다 클릭한 번호가 더 크면 : 다음 슬라이드 메인함수
                            //현재 슬라이드번호 보다 클릭한 번호가 더 작으면 : 이전 슬라이드 메인함수
                            timerControlFn(); //타이머 함수 실행

                            imsi = cnt; //현재 실행중인 슬라이드 임시 기억
                            cnt = idx;  //클릭한 번호 이동하고자하는 번호

                            if( imsi < idx ) { //다음 슬라이드                               
                                mainNextSlideFn();
                            }
                            else if( imsi > idx  ) { //이전 슬라이드                                
                                mainPrevSlideFn();
                            }
                    ...
                    setId2 = setInterval(function(){
                        tCnt++;
                            if( tCnt >= 5 ){
                                ...
                                clearInterval(setId2);
                                nextCountFn(); //즉시 다음카운트
                                autoPlayInit(); //4초 후에 자동 실행
                            }
                    },1000);
                }
